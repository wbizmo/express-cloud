  ...
  <div class="ec-responsive-table overflow-x-auto"><table class="w-full min-w-[860px] text-left text-sm"><thead><tr>
  ...
  <th class="px-3 py-3">Store Credit</th>
  <th class="px-3 py-3"></th>
  </tr></thead><tbody>@forelse($customers as $customer)<tr class="border-t border-slate-100"><td class="px-3 py-4 font-semibold">{{ $customer->name }}</td><td class="px-3 py-4">{{ $customer->phone }}</td><td class="px-3 py-4 font-semibold {{ (int)$customer->outstanding_kobo > 0 ? 'text-red-700' : 'text-emerald-700' }}">₦{{ number_format(((int)$customer->outstanding_kobo)/100,2) }}</td><td class="px-3 py-4 font-semibold {{ $customer->storeCreditKobo() > 0 ? 'text-emerald-700' : 'text-slate-400' }}">{{ $customer->storeCreditKobo() > 0 ? '₦'.number_format($customer->storeCreditKobo()/100,2).' owed to them' : '—' }}</td><td class="px-3 py-4 text-right"><a class="font-semibold text-slate-900" href="{{ route('admin.commercial.receivables.show',$customer) }}">Open account</a></td></tr>@empty<tr><td colspan="5" class="px-3 py-10 text-center text-slate-500">No customers found.</td></tr>@endforelse</tbody></table></div>
