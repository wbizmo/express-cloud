...
final readonly class LisaConversationEngine
{
    ...
    public function answer(Account $actor, LisaConversation $conversation, string $question): array
    {
        ...
        $reply = match (true) {
            Str::contains($normalized, ['how do i create a sale', 'how to create a sale']) => 'Open Create Sale, select the branch you are transacting from, optionally select or add a customer, add products with the product selector or barcode scanner, confirm quantities and payment, then complete the transaction.',
            Str::contains($normalized, ['how do i transfer stock', 'how to transfer stock']) => 'Open Inventory, choose Stock Transfer, select the source branch, destination branch and product, verify current and projected balances, enter the quantity and reference note, then submit.',
            Str::contains($normalized, ['how do i add a customer', 'how to add a customer']) => 'From Create Sale or POS, use New Customer beside the customer selector. Only the name is required. A blank customer remains a walk-in sale.',
            ...
            Str::contains($normalized, ['purchase order', 'purchases', 'procurement']) => sprintf(
            ...
            default => "I understand your request, {$name}. I can answer questions about sales, customers, inventory, purchasing, reports, staff performance and how to operate Express Cloud. Ask for a metric, branch comparison or step-by-step workflow.",
        };
