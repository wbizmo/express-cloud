...
if rg -n "product_branch_stocks|reorder_level_milliunits" app routes resources database/migrations --glob '*.php'; then
  echo "[error] A legacy stock-table identifier still exists. Review the lines above." >&2
  exit 1
fi
if rg -n "sales.*balance_due_kobo|where\('balance_due_kobo'|sum\('balance_due_kobo'" app resources --glob '*.php'; then
...
# Role writes are constrained by role family, while unknown custom roles use the exact chosen permissions.
python3 <<'PY'
from pathlib import Path
p=Path('app/Http/Controllers/Admin/RoleController.php')
if p.exists():
    s=p.read_text()
    if 'RolePermissionPolicy' not in s:
        anchor='use App\\Support\\Authorization\\PermissionCatalog;'
        s=s.replace(anchor, anchor+'\nuse App\\Support\\Authorization\\RolePermissionPolicy;')
    s=s.replace("$request->array('permissions')", "RolePermissionPolicy::constrain(\n                    $request->string('name')->toString(),\n                    $request->string('slug')->toString(),\n                    $request->array('permissions'),\n                )")
    p.write_text(s)
PY
...
echo "[10/12] Fixing top bar, body, cards and two-column overflow without clipping content"
cat >> resources/css/app.css <<'CSS'
...
echo "[11/12] Moving the web entry point to repository root while preserving Vite assets in public"
...
echo "[12/12] Validating, rebuilding and packaging"
php_files=(
  app/Services/Dashboard/StaffDashboardData.php
  app/Services/Organisation/AuthorizationService.php
...
! rg -n "product_branch_stocks|reorder_level_milliunits" app routes resources database/migrations --glob '*.php'
